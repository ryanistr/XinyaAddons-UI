...
class SystemBootReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Intent.ACTION_BOOT_COMPLETED) {
            Log.d("SystemBootReceiver", "Boot completed, restoring system configurations...")
            restoreDisplayConfig()
        }
    }

    private fun restoreDisplayConfig() {
        Thread {
            try {
                // Wait a bit to ensure SurfaceFlinger is ready
                Thread.sleep(5000)
                ...
                if (parts.size >= 4) {
                    ...
                    val saturationVal = parts[3].toFloatOrNull() ?: 1000f
                    ...
                    val s = saturationVal / 1000f
                    val satCmd = "service call SurfaceFlinger 1022 f $s"
                    Runtime.getRuntime().exec(satCmd).waitFor()
                    
                    Log.d("SystemBootReceiver", "Restored Display Config: Color($r,$g,$b), Saturation($s)")
                }
            } catch (e: Exception) {
                Log.e("SystemBootReceiver", "Failed to restore display config", e)
            }
        }.start()
    }
}
